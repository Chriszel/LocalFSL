...
from tensorflow.keras.layers import LSTM, Dropout, Dense, Input, TimeDistributed, GlobalAveragePooling2D
...
# --------------------------
def load_data():
    """Loads dataset from .npy files and prepares for training."""
    try:
...
# --------------------------
# ✅ 3. K-Fold Data Generator
# --------------------------
class VideoDataGenerator(tf.keras.utils.Sequence):
    """Efficiently loads video batches and resizes frames to 224x224."""
...
def build_transfer_learning_model(input_shape, num_classes):
    """Builds a transfer learning model using MobileNetV2 + LSTM."""
...
# ✅ 5. K-Fold Training
# --------------------------
def train_kfold(X_train, y_train, num_classes, n_splits=5, epochs=50, batch_size=16):
...
def main():
    ...
    fold_accuracies = train_kfold(X_train, y_train, num_classes, n_splits=5, epochs=50, batch_size=16)
    ...
    final_model.compile(optimizer=optimizers.Adam(learning_rate=1e-4), loss='categorical_crossentropy', metrics=['accuracy'])

    final_model.fit(VideoDataGenerator(X_train, y_train, batch_size=16, augment=True), epochs=50, verbose=1)

    # ✅ Now we can evaluate the model because it's defined
